...
function Initialize()
	...
	secondTextEndOffset = core.Vector(650, 500)

	thirdTextBeginOffset = core.Vector(ScreenWidth , 500)
	thirdTextEndOffset = core.Vector(1000, 500)
...
function Draw ()
	...
	ClientSettings.Fullscreen = ui.CheckBox(ClientSettings.Fullscreen, core.Vector(pos.X + optionSpacer, pos.Y), textColor, textShadowColor, mouseOverColor, true)
	pos.Y = pos.Y + horizontalSpacer
	DrawText(font, 'Vertical sync', core.Vector(pos.X, pos.Y), textColor)
	ClientSettings.Vsync = ui.CheckBox(ClientSettings.Vsync, core.Vector(pos.X + optionSpacer, pos.Y), textColor, textShadowColor, mouseOverColor, true)
	pos.Y = pos.Y + horizontalSpacer
	...
	ClientSettings.ConsoleEnabled = ui.CheckBox(ClientSettings.ConsoleEnabled, core.Vector(pos.X + optionSpacer, pos.Y), textColor, textShadowColor, mouseOverColor, true)
	pos.Y = pos.Y + horizontalSpacer
	DrawText(font, 'Console timestamps', core.Vector(pos.X, pos.Y), textColor)
	ClientSettings.LogTimestamp = ui.CheckBox(ClientSettings.LogTimestamp, core.Vector(pos.X + optionSpacer, pos.Y), textColor, textShadowColor, mouseOverColor, true)
	pos.Y = pos.Y + horizontalSpacer
	DrawText(font, 'Log console output', core.Vector(pos.X, pos.Y), textColor)
	ClientSettings.EnableLog = ui.CheckBox(ClientSettings.EnableLog, core.Vector(pos.X + optionSpacer, pos.Y), textColor, textShadowColor, mouseOverColor, true)
	pos.Y = pos.Y + horizontalSpacer
	DrawText(font, 'Open log on exit', core.Vector(pos.X, pos.Y), textColor)
	ClientSettings.OpenLogOnClose = ui.CheckBox(ClientSettings.OpenLogOnClose, core.Vector(pos.X + optionSpacer, pos.Y), textColor, textShadowColor, mouseOverColor, ClientSettings.EnableLog)
	pos.Y = pos.Y + horizontalSpacer
	DrawText(font, 'Persistent history', core.Vector(pos.X, pos.Y), textColor)
	ClientSettings.ConsolePersistentHistory = ui.CheckBox(ClientSettings.ConsolePersistentHistory, core.Vector(pos.X + optionSpacer, pos.Y), textColor, textShadowColor, mouseOverColor, true)
	pos.Y = pos.Y + horizontalSpacer
	...
	masterVolume = ui.Slider(core.Vector(pos.X + optionSpacer, pos.Y), 200, masterVolume, textColor, textShadowColor, mouseOverColor, true)
	pos.Y = pos.Y + horizontalSpacer
	DrawText(font, 'Music Volume', core.Vector(pos.X, pos.Y), textColor)
	musicVolume = ui.Slider(core.Vector(pos.X + optionSpacer, pos.Y), 200, musicVolume, textColor, textShadowColor, mouseOverColor, true)
	pos.Y = pos.Y + horizontalSpacer
	DrawText(font, 'Ambience Volume', core.Vector(pos.X, pos.Y), textColor)
	ambienceVolume = ui.Slider(core.Vector(pos.X + optionSpacer, pos.Y), 200, ambienceVolume, textColor, textShadowColor, mouseOverColor, true)
	pos.Y = pos.Y + horizontalSpacer
end
--Copyright d4, 2017--